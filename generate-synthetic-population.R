...
df_synth_pop = distr_agent_neigh_age_group(neigh_df = df_MarginalDistr,
...
for (neighb_code in unique(df_synth_pop$neighb_code)) {
  for (i in 1:nrow(df_StratEduCurrent)) {
    agents_neighbourhood = df_synth_pop[df_synth_pop$neighb_code == neighb_code & df_synth_pop$gender == df_StratHousehold[i, 'gender'] &
                                          df_synth_pop$age_group == df_StratHousehold[i, 'age_group'] &
                                          df_synth_pop$migration_background == df_StratHousehold[i, 'migration_background'],]
